...
tcRnModule hsc_env keep_lbls pmod = do
  ...
  ((tc_gbl_env', mrn_info), splices, mods)
      <- liftIO $ captureSplicesAndDeps (hscSetFlags (ms_hspp_opts ms) hsc_env) $ \hsc_env_tmp ->
          ...
          go seen new
            ...
            | otherwise = go seen' new'
              where
                ...
                new_deps = unionManyUniqSets [ mkUniqSet $ map fst $ dep_mods $ mi_deps $ hm_iface mod_info
